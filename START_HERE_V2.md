...
4. קרא את `docs/V2_INTEGRATION_PROTOCOL_HE.md` אם אתה מכניס עבודה שנעשתה בצד או נוגע ברישוי/מודולים.
5. בחר משימה אחת בלבד עם `status: todo` ו־`owner` מתאים.
6. אם scope הקבצים מתנגש עם משימה `in_progress` — דלג.
7. עדכן את המשימה ל־`in_progress` לפני עבודה.
8. אל תכתוב קוד לפני שיש owner, input, output, DB, API, events, screens ו־Definition of Done.

## כללי V2

- לא מוסיפים קוד עסקי ל־`server.js`.
- לא מערבבים מודולים.
- מאחדים רעיונות ו־flow למשתמש, אבל מפרידים ownership, נתונים, API וקוד לפי מודולים.
...
- `docs/V2_INTEGRATION_PROTOCOL_HE.md` — שער קליטה לעבודות Claude/GPT/רישוי בלי טלאים.
- `docs/admin-system-licensing-boundaries.md` — גבולות Admin/System/Licensing: שליטה ונראות, לא בעלות עסקית.
