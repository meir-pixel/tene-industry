...
- שם האלמנט כפי שהלקוח תיאר אותו, למשל קורה 1, עמוד מערבי, רתמה או קורה תחתית.
- תיאור צורה בשפת הלקוח, למשל ח, ספסל, ליפט, חישוק, ספירלה או מוט ישר.
- צורה ויזואלית ברורה.
- קוטר ברזל.
- כמות.
...
זווית 90° מסומנת בסימן ריבוע פינה בלבד — אין לכתוב טקסט "90°" לצד הסימן. טקסט מעלות מודפס רק לזוויות שאינן 90°.
...
## חוק תצוגה למסמכים מרובי שורות

בהזמנות ברזל אמיתיות יכולים להיות עשרות פריטים על פני כמה עמודים. לכן אסור להציג כרטיס גדול לכל שורה במסך ההשוואה הראשי.

כלל UI:

- הצד של הפענוח מוצג כטבלה צפופה עם שורות רבות, סינון וסטטוס לכל שורה.
- בהזמנות שמגיעות כצורות/כרטיסיות, הטבלה מיושרת לפורמט כרטיסיית ייצור: מס' פריט, שם האלמנט, תיאור צורה, צורה ומידות, קוטר, כמות, אורך, סה"כ אורך, משקל וסטטוס בדיקה.
- `element_name` שומר את שם האלמנט כפי שהלקוח כתב. `shape_description` שומר את תיאור הצורה במילים של הלקוח. `shape_name` נשאר שם צורה קנוני לעורך/מכונה.
- כל שורה נשמרת/מסומנת בנפרד כ־`review_status=accepted`, כדי שהמשתמש יראה בדיוק אילו שורות כבר נבדקו.
- רק השורה שנבחרה נפתחת ככרטיס/פאנל פרטים עם שרטוט גדול, שדות עריכה ואישור נקודתי.
- מסמך המקור נשאר לצד הטבלה, עם אפשרות זום וגלילה לפי עמוד/שורה.
- לחיצה על צורה בטבלה או בפאנל פותחת עורך צורה.
- עבור מסמך סרוק ללא טקסט, המסך עדיין מציג מקור מלא, אך הפענוח נשאר טיוטה עד בדיקה אנושית.
...
- For bent rows, the shape geometry comes from sketch side dimensions. If side dimensions do not match total cut length, keep the geometry and mark the row for review instead of silently changing the shape.
