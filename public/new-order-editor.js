(function () {
  ...
  function pileCageLengths(item = {}) { const snapshot = lineContract(item) || item.shapeSnapshot || {}; const data = snapshot.data || {}; const calculated = snapshot.calculated || {}; return { cageLengthMm: numeric(data.pileLengthMm ?? data.pileLength ?? item.length, 0), steelLengthMm: numeric(calculated.totalLengthMm, 0) }; }
  function getLineUnitLengthMm(item = {}) { const data = lineData(item); const calculated = lineContract(item)?.calculated || item.shapeSnapshot?.calculated || {}; if (isRoundPileCageLine(item)) return pileCageLengths(item).cageLengthMm; if (isLineSpiral(item)) { const spiral = getSpiralFields(item); if (typeof window.spiralLengthMm === 'function' && spiral.spiralDiameterMm > 0 && spiral.spiralTurns > 0) return window.spiralLengthMm(spiral.spiralDiameterMm, spiral.spiralTurns); } const sides = lineSides(item); const sideTotal = sides.reduce((sum, length) => sum + numeric(length, 0), 0); if (sideTotal > 0) return sideTotal; return numeric(item.length ?? item.totalLengthMm ?? data.lengthMm ?? data.totalLengthMm ?? calculated.unitLengthMm ?? calculated.totalLengthMm, 0); }
  function formatLineLength(item = {}) { return formatMm(getLineUnitLengthMm(item)); }
  function formatLineTotalLength(item = {}) { const unitLengthMm = isRoundPileCageLine(item) ? pileCageLengths(item).steelLengthMm : getLineUnitLengthMm(item); return formatMeters(unitLengthMm * lineQty(item)); }
