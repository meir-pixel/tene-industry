...
- `pricing_price_books` - כותרת מחירון, לקוח, סוג, סטטוס ותאריך עדכון אחרון.
...
| --- | --- | --- |
| מחירון כללי | `general` | ברירת מחדל ללקוח שאין לו מחירון אישי פעיל. |
...
- יצירת מחירון חדש.
- עריכת כותרת מחירון: קוד, שם, מספר לקוח במסלול מחירון לקוח, סוג, מטבע וסטטוס.
...
- הצגת `updated_at` אוטומטי. אין תאריך ידני למחירון; כשיש עדכון, זה תאריך העדכון.

תנאי תשלום אינם חלק ממחירון. הם שייכים למודול Customers / Payment Terms.
מחירון כללי נשמר ללא `customer_id`; מחירון לקוח חייב להתחבר ללקוח דרך `customer_id`.
