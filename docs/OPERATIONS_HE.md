...
## 📋 משימות פתוחות (אפיון מוכן, ממתין לביצוע)

> 🟢 **הלוח החי למשימות הוא [`TASKS.md`](../TASKS.md) בשורש הריפו** — שם מסמנים ומשם הסוכנים לוקחים. הטבלה למטה היא רקע/הקשר בלבד.

| # | משימה | אפיון | הערה קריטית |
|---|-------|-------|-------------|
| 1 | ספירת משתמשים פעילים | spec-license-modules.md | `license.js` שולח `activeUsers` ל-/api/check — אחרת מכסת המשתמשים לא עובדת |
| 2 | ייצוא ל-Priority | [spec-priority-export.md](spec-priority-export.md) | Priority רוצה סוג פריט+משקל+מחיר, לא צורות כיפוף |
| 3 | שני סוגי מחירון | [spec-dual-pricing.md](spec-dual-pricing.md) | מחירון שלי / מחירון הלקוח, נקלט מתמונה/קובץ/ידני |
