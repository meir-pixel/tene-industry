...
אם חסר מחיר אישי לקוטר:

```json
{
  "diameter": 14,
  "unitPrice": 3.9,
  "pricingSource": "general_fallback",
  "pricingLabel": "מחירון כללי זמני",
  "warnings": ["customer_price_missing"]
}
```
...
- אם חסר מחיר אישי, הפורטל לא מציג מחיר כרגיל; הוא מציג אזהרה ודורש אישור מנהל לפני הפיכת ההצעה להזמנה.
